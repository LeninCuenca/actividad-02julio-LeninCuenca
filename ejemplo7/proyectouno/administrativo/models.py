...
class Modulo(models.Model):
    ...
    costo_matricula = models.DecimalField(max_digits=8, decimal_places=2, default=100.00)  # Nuevo campo
    estudiantes = models.ManyToManyField(Estudiante, through='Matricula')

    def __str__(self):
        return "Módulo: %s" % (self.nombre)

    def valor_total_matriculas(self):
        return sum(m.costo for m in self.lasmatriculas.all())
...
class Matricula(models.Model):
    ...
    costo = models.DecimalField(max_digits=8, decimal_places=2, default=0) # Agregar costo
